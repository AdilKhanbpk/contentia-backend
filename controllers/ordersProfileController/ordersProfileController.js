...
exports.updateOrdersProfile = catchAsync(async (req, res) => {
    ...
    if (!ordersProfileData) {
        return res.status(400).json({ success: false, message: 'OrdersProfile data is required.' });
    }

    // Ensure an _id is provided for existing OrdersProfile
    if (!ordersProfileData._id) {
        return res.status(400).json({ success: false, message: 'OrdersProfile ID is required.' });
    }
    ...
    await User.updateOne({ _id: req.user.id }, { ordersProfile: ordersProfile._id });
...
exports.getUserWithProfile = catchAsync(async (req, res) => {
