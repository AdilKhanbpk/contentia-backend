...
export const creatorforgotPassword = asyncHandler(async (req, res) => {
    const { email } = req.body;

    if (!email) {
        throw new ApiError(400, "Creator Email is required");
    }

    const user = await Creator.findOne({ email });
    if (!user) {
        throw new ApiError(404, "Creator email not found");
    }
    ...
    const absoluteResetURL = `${baseUrl}/reset-creator-password/${token}`;
