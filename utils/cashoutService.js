...
const generateHashKey = (
  ...
  const msg_encrypted_bundle = `${iv.toString(
    "hex"
  )}:${salt}:${encrypted}`.replace(/\//g, "__");
...
const cashoutToBank = async (cashoutData) => {
  try {
    ...
    console.log(
      "Request body prepared for cashout:",
...
export { generateHashKey, getSipayToken, cashoutToBank };
